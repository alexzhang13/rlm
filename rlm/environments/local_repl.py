...
_SAFE_BUILTINS = {
    ...
    "sorted": sorted, "reversed": reversed, "range": range, "min": min,
    "max": max, "sum": sum, "abs": abs, "round": round, "any": any,
    "all": all, "pow": pow, "divmod": divmod, "chr": chr, "ord": ord,
    "hex": hex, "bin": bin, "oct": oct, "repr": repr, "ascii": ascii,
    "format": format, "hash": hash, "id": id, "iter": iter, "next": next,
    "slice": slice, "callable": callable, "hasattr": hasattr, "getattr": getattr,
    ...
    "NameError": NameError, "ImportError": ImportError, "StopIteration": StopIteration,
    ...
    "input": None, "eval": None, "exec": None, "compile": None, "globals": None, "locals": None,
}
...
class BaseLocalREPL(NonIsolatedEnv):
    ...
    def add_context(self, context_payload: dict | list | str, context_index: int | None = None) -> int:
        ...
        if isinstance(context_payload, str):
            ...
            with open(context_path, "w") as f: f.write(context_payload)
            self.execute_code(f"with open(r'{context_path}', 'r') as f:\n    {var_name} = f.read()")
        else:
            context_path = os.path.join(self.temp_dir, f"{var_name}.json")
            with open(context_path, "w") as f: json.dump(context_payload, f)
    ...
    def get_context_count(self) -> int: return self._context_count

    def add_history(self, message_history: list[dict[str, Any]], history_index: int | None = None) -> int:
        if history_index is None: history_index = self._history_count
        var_name = f"history_{history_index}"
        self.locals[var_name] = copy.deepcopy(message_history)
        if history_index == 0: self.locals["history"] = self.locals[var_name]
        self._history_count = max(self._history_count, history_index + 1)
        return history_index

    def get_history_count(self) -> int: return self._history_count
    ...
    def execute_code(self, code: str) -> REPLResult:
        ...
        with self._capture_output() as (stdout_buf, stderr_buf):
            try:
                ...
                stdout, stderr = stdout_buf.getvalue(), stderr_buf.getvalue() + f"\n{type(e).__name__}: {e}"
        return REPLResult(stdout=stdout, stderr=stderr, locals=self.locals.copy(), 
                          execution_time=time.perf_counter() - start_time, rlm_calls=self._pending_llm_calls.copy())

    def cleanup(self):
        try: shutil.rmtree(self.temp_dir)
        except Exception: pass
        self.globals.clear(); self.locals.clear()
    ...
    @property
    def trace_id(self) -> str | None: return self.metadata.get("trace_id")
    
    def _llm_query(self, *args, **kwargs): raise NotImplementedError
    def _llm_query_batched(self, *args, **kwargs): raise NotImplementedError
...
class DirectREPL(BaseLocalREPL):
    ...
    def _llm_query(self, prompt, model=None, response_format=None, tools=None, tool_handler=None, response_model=None) -> Any:
        ...
        for _ in range(MAX_TOOL_ITERATIONS):
            request = LMRequest(prompt=messages, model=model, depth=self.depth, 
                               response_format=response_format, tools=tools, metadata=self.metadata)
            response = self.lm_handler.handle_request(request)
            if not response.success: return f"__LLM_ERROR__|handler_error|0|{response.error}"
            cc = response.chat_completion
            if cc.error: return f"__LLM_ERROR__|{cc.error_type}|{cc.status_code}|{cc.error}"
            content = cc.response
            if tools and content.startswith("{") and "tool_calls" in content:
                ...
                for t in tc:
                    try: res = tool_handler(t["name"], t["arguments"])
    ...
    def _llm_query_batched(self, prompts, model=None, response_formats=None, tools=None, tool_handler=None, response_models=None) -> list[Any]:
        ...
        for cc in response.chat_completions:
            if cc.error: res.append(f"Error: {cc.error}")
            else: self._pending_llm_calls.append(cc); res.append(cc.response)
        return res
...
class SocketREPL(BaseLocalREPL):
    ...
    def _llm_query(self, prompt, model=None, response_format=None, tools=None, tool_handler=None, response_model=None) -> Any:
        ...
        for _ in range(MAX_TOOL_ITERATIONS):
            ...
            if not resp.success: return f"__LLM_ERROR__|socket_error|0|{resp.error}"
    ...
    def _llm_query_batched(self, prompts, model=None, response_formats=None, tools=None, tool_handler=None, response_models=None) -> list[Any]:
        ...
        try:
            ...
            res = []
            for r in resps:
                if not r.success: res.append(f"Error: {r.error}")
                else:
                    cc = r.chat_completion
                    if cc.error: res.append(f"Error: {cc.error}")
                    else: self._pending_llm_calls.append(cc); res.append(cc.response)
            return res
        except Exception as e: return [f"Error: {e}"] * len(prompts)

    def update_handler_address(self, address): self.lm_handler_address = address


class DillREPL(DirectREPL):
    def save_state(self, path):
        if dill is None: raise ImportError("dill required")
        clean = {k: v for k, v in self.locals.items() if not k.startswith("_") and not callable(v)}
        with open(path, "wb") as f: dill.dump(clean, f)
    def load_state(self, path):
        if dill is None: raise ImportError("dill required")
        if os.path.exists(path):
            with open(path, "rb") as f: self.locals.update(dill.load(f))
