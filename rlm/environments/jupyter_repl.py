...
class JupyterREPL(NonIsolatedEnv):
    ...
    def __init__(
        ...
        sync_from_user_ns: bool = False,
        sync_vars: list[str] | None = None,
        **kwargs,
    ):
        ...
        self.sync_from_user_ns = sync_from_user_ns
    ...
    def pull_user_namespace(self) -> None:
        """Pull variables from the user namespace into the REPL."""
        if not self.sync_from_user_ns and not self.sync_vars:
            return
        ...
        # If sync_vars is provided, only sync those variables
        if self.sync_vars:
            for var in self.sync_vars:
                if var in ip.user_ns:
                    self.locals[var] = ip.user_ns[var]
            return

        # Otherwise, sync everything if sync_from_user_ns is True
        if self.sync_from_user_ns:
            for key, value in ip.user_ns.items():
                if key.startswith("_") or key in self.globals:
                    continue
                # Do not overwrite context variables managed by RLM
                if key.startswith("session_context_") or key == "context_history":
                    continue
                self.locals[key] = value
