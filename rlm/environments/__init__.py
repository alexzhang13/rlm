...
def get_environment(
    environment: Literal["local", "modal", "docker", "daytona", "prime", "e2b", "monty"],
    environment_kwargs: dict[str, Any],
) -> BaseEnv:
    """
    Routes a specific environment and the args (as a dict) to the appropriate environment if supported.
    Currently supported environments: ['local', 'modal', 'docker', 'daytona', 'prime', 'e2b', 'monty']
    """
    if environment == "local":
        ...
        raise ValueError(
            ...
            "['local', 'modal', 'docker', 'daytona', 'prime', 'e2b', 'monty']"
        )
