...
#### Apple Container (*requires Apple's `container` CLI on macOS*)
`AppleContainerREPL` runs Python code with Apple's `container` CLI using the same host-routed `llm_query` bridge as `DockerREPL`. Use `environment="apple"` and optionally pass `environment_kwargs={"image": "python:3.11-slim"}`. If `llm_query` calls need to reach the host, configure Apple's localhost DNS once with `sudo container system dns create host.container.internal --localhost 203.0.113.113`, or pass a custom `host_proxy_name`.
...
### Model Providers
We currently support most major clients (OpenAI, Anthropic), as well as the router platforms (OpenRouter, Portkey). For local models, we recommend using vLLM (which interfaces with the [OpenAI client](https://github.com/alexzhang13/rlm/blob/main/rlm/clients/openai.py)). On Apple Silicon, you can also install `pip install "rlms[mlx]"` and use `backend="mlx"` with `backend_kwargs={"model_name": "<local-path-or-hf-id>"}` for local `mlx-vlm` inference. To view or add support for more clients, start by looking at [`rlm/clients/`](https://github.com/alexzhang13/rlm/tree/main/rlm/clients).
