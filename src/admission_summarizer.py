...
class AdmissionSummarizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized AdmissionSummarizer with model: {model_name} (new SDK: {USE_NEW_SDK})"
        )

    def summarize(
        self, discharge_text: str, metadata: dict[str, Any], validate: bool = False
    ) -> dict[str, Any]:
        """
    ...
    def _call_api_with_retry(self, prompt: str) -> dict[str, Any] | None:
    ...
    def _validate_summary(
        self, discharge_text: str, extracted_summary: dict[str, Any]
    ) -> dict[str, Any]:
        """Run validation pass on extracted summary."""
        prompt = VALIDATION_PROMPT.format(
            discharge_text=discharge_text, extracted_summary=json.dumps(extracted_summary, indent=2)
        )
    ...
    def summarize_batch(
        ...
        for i, record in enumerate(records):
            ...
            summary = self.summarize(discharge_text=record.get("input", ""), metadata=metadata)
...
def create_summarizer(api_key: str | None = None) -> AdmissionSummarizer:
